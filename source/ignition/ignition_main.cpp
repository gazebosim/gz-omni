...
// export
// LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/ahcorde/.local/share/ov/pkg/connectsample-103.1.0/_build/linux-x86_64/debug/:/home/ahcorde/.local/share/ov/pkg/connectsample-103.1.0/_build/linux-x86_64/release/
// bash build.sh
// ./_build/linux-x86_64/debug/ignition
...
static std::string createOmniverseModel(const std::string& destinationPath)
{
  ...
  // Delete the old version of this file on Omniverse and wait for the operation
  // to complete
  {
    std::unique_lock<std::mutex> lk(gLogMutex);
...
int main(int argc, char* argv[])
{
  ...
  auto usdCylinder =
      pxr::UsdGeomCylinder::Define(gStage, pxr::SdfPath("/World/Cylinder"));
  ...
  while (true)
  {
    ...
    // Process any updates that may have happened to the stage from another
    // client omniUsdLiveWaitForPendingUpdates();
  ...
  // Calling this prior to shutdown ensures that all pending live updates
  // complete.
  omniUsdLiveWaitForPendingUpdates();
