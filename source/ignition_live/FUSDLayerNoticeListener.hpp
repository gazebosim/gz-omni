...
{
 public:
  FUSDLayerNoticeListener(Scene& _scene, const std::string& _worldName)
      ...
      const pxr::TfWeakPtr<pxr::SdfLayer>& _sender)
  {
    ...
    for (const auto& changeEntry : iter->second.GetEntryList())
    {
      const pxr::SdfPath& sdfPath = changeEntry.first;

      igndbg << "got change notification [" << sdfPath << "]" << std::endl;

      pxr::SdfPathVector vectorPath = sdfPath.GetPrefixes();

      if (changeEntry.second.flags.didRemoveNonInertPrim)
      {
        igndbg << "didRemoveNonInertPrim " << sdfPath.GetName() << std::endl;
      ...
      {
        igndbg << "didAddNonInertPrim " << sdfPath.GetName() << std::endl;
      }
      ...
      //         {
      //           pxr::GfVec3d translated =
      //           Info.second.second.Get<pxr::GfVec3d>(); translate[0] =
      //           translated[0]; translate[1] = translated[1]; translate[2] =
      //           translated[2];
      //         }
      ...
      //         bool result;
      //         unsigned int timeout = 5000;
      //         bool executed = node.Request("/world/" + worldName +
      //         "/set_pose",
      //                                      req, timeout, rep, result);
      //         if (executed)
      //         {
      ...
      //         // 	"/world/" + worldName + "/set_pose", req, timeout, rep,
      //         result);
