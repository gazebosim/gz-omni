...
namespace ignition::rendering::omni
{
  ...
  public:
    ...
    std::map<std::string, math::Matrix4d> SkeletonLocalTransforms() const override;
    void SetSkeletonLocalTransforms(const std::map<std::string, math::Matrix4d> &_tfs) override;
    std::unordered_map<std::string, float> SkeletonWeights() const override;
    void SetSkeletonWeights(const std::unordered_map<std::string, float> &_weights) override;
    void SetSkeletonAnimationEnabled(const std::string &_name, bool _enabled, bool _loop = true, float _weight = 1.0) override;
    bool SkeletonAnimationEnabled(const std::string &_name) const override;
    void UpdateSkeletonAnimation(std::chrono::steady_clock::duration _time) override;
